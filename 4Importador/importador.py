...
def spectogram_and_save(filename):
    ...
    #FRAMES = 299
    FRAMES = 149
    assert len(spectrogram) == FRAMES * 41

    spectrogram = spectrogram.reshape(FRAMES,41)
...
for label in labels:
    ...
    for i in range(99, 100):
        ...
        spectrogram_esp32 = spectogram_and_save(f"DATASET/{label}/{label}_{i}.png")
        spectrogram_tensorFlow = create_tf_spectrogram( wav_filename, f"DATASET/{label}/{label}_{i}_tf.png")
        compare_spectrograms( spectrogram_tensorFlow, spectrogram_esp32, f"DATASET/{label}/{label}_{i}_compare.png")
