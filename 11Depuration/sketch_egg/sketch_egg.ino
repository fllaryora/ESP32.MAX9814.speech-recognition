...
const char* kCategoryLabels[kCategoryCount] = {"0_Ayuda", "1_Listo", "2_No", "3_Papel", "4_Si"};
/*
...
void loop() {
  ...
  if(voiceDetected) {
    ...
    for(int i = 0; i < 3; i++)
    {
      Serial.printf( "%d) %s | raw=%d | prob=%f\n", i + 1, kCategoryLabels[results[i].index],results[i].raw, results[i].probability);
    }
